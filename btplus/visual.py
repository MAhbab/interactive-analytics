...
def result_panel(res, display=True):
    """
    A bokeh panel to visualize backtest results.
    
    Args:
        res (Result): Backtest results
        display (bool): If True, displays the panel. Otherwise, the panel is returned. The latter
         is useful for tabulating multiple results into the same graphic object.
    ...
    Examples:
        The preferred way to call this function is as a member function of ``bt.Result``. See below.
    ...
    metrics['Performance'] = ['start', 'end', 'rf', 'total_return', 'cagr',
                    'mtd', 'three_month', 'six_month', 'ytd', 'one_year',
                    'three_year', 'five_year', 'ten_year', 'incep']

    metrics['Returns'] = ['cagr', 'daily_sharpe', 'daily_sortino', 'daily_mean', 
                    'daily_vol', 'daily_skew', 'daily_kurt']

    metrics['Risk'] = ['max_drawdown', 'avg_drawdown', 'avg_drawdown_days',
                        'best_day', 'worst_day', 'best_month', 'worst_month', 
                        'best_year', 'worst_year', 'twelve_month_win_perc']

    dfs = {k: stats.loc[metrics[k]] for k in metrics}
    tables_list = [Panel(title=k, child=dfs[k].to_bokeh_table(width=900, height=300)) for k in dfs]
    table = Tabs(tabs=tables_list)

    price_plot = Panel(title='Prices', child=price.bokeh_time_series(xlabel='Time', ylabel='Price', title='Backtest Performance', width=900, height=400))

    plots = Tabs(tabs=[price_plot])

    out = column([plots, table])
    if display:
        return show(out)
    return out
...
def result_hiplot(res):
    """
    Generates a HiPlot object from a backtest results. See https://github.com/facebookresearch/hiplot 
    for more information. 
    ...
    Example:
        ...
        This function can also be called as the member function of a `bt.Result` instance as shown below.
        Note that btplus must be imported.
    ...
    """
    price = res.prices
    log_rets = np.log(price/price.shift()).dropna()
    log_rets['Epoch Time'] = log_rets.index.astype('int64')/1e9
    log_rets = log_rets.reset_index().astype(str)

    return log_rets.hiplot()
...
def extend_objects():
    ...
    Result.panel = result_panel
    Result.hiplot = result_hiplot
